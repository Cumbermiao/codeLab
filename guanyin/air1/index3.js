~(function() {
  ...
  var getDetailDataByIDAndCb = function(id, cb) {
      ...
      axios
          ...
          .then(function(resp) {
              ...
              detailData = renderData.filter(item=>item.id === id)[0];
              cb();
              // //renderByData 数据渲染
              // if ("update" == target) {
          ...
          .bootstrapTable({
              ...
              onPostBody: function(data) {
                  ...
                  $(".showa").click(function() {
                      ...
                      //数据回显
                    //TODO:
                    // var a = data.filter(item=>item.id === id)
                    // console.log(a)
                    // detailData = a[0]
  ...
  $(function() {
      ...
      function generateTabContentTemp(arr, cb, type) {
          ...
          for (var i = 0; i < arr.length; i++) {
              var labelKey =  arr[i].classifyValue || "";
              var label = arr[i].initFlag === 'I' ? arr[i].airlineDesc : labelObj[labelKey];
              var segments;
      ...
      var detailModal = {
          ...
          hooks: {
              ...
              updated: noop, // 组件state UI 已经更新， 用于 template 结构变化后， 重新监听事件
              beforeDestory: noop, //卸载之前, 此处用来卸载事件
              destoryed: function(){ console.log('detail destory' )} //组件已卸载
          }
      };
