~(function() {
    $(function(){
        ...
        $("#modalUpdate").on('click', function (e) {
            ...
            var seatArr = [], inlandObj = {}, nationObj = {};
            ...
            for (var i = 0; i < seatArr.length; i++) {
                if(parseInt(seatArr[i].holdBackBusinessNum)> parseInt(seatArr[i].businessClassSeatNum)){
                    alert(seatArr[i].planeTypeName+"活动提前关闭时座位数请小于公务舱总座位数");
                    flag = false
                }
            ...
            if(flag){
                ...
                layer.confirm('修改此参数可能会对现有活动造成影响，是否确定继续进行修改？', {
                    //按钮
                    btn: ['确定', '取消']
                },function(index){
                    //新增提交
                    $.ajax({
                        ...
                        data: req,
                        success: function (resp) {       //data回调信息
                            layer.close(index);
                            if (resp.status == 200) {
                                //成功
    ...
    var $seatTable, $priceTable, $table;
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row) {
                            if("F" == row.status){
                                //禁用状态展示启用
                                return '<button type="button" class="btn btn-outline-secondary showa" style="margin: 0px 2px;font-size: inherit;" id="show" data-id="' + row.id+ '" >查看</button>' +
                                    '<button type="button" class="btn btn-outline-primary update" style="margin: 0px 2px;font-size: inherit;" id="update" data-id="' + row.id+ '" >修改</button>' +
                                    '<button type="button" class="btn  btn-danger enable" style="margin: 0px 2px;font-size: inherit;" id="enableById" data-id="' + row.id+ '" >未启用</button>' +
                                    '<button type="button" class="btn btn-outline-danger delete" style="margin: 0px 2px;font-size: inherit;" id="deleteById" data-id="' + row.id + '">删除</button>';
                            }else if("T" == row.status){
                                //启用状态展示禁用
                                return '<button type="button" class="btn btn-outline-secondary showa" style="margin: 0px 2px;font-size: inherit;" id="show" data-id="' + row.id+ '" >查看</button>' +
                                    '<button type="button" class="btn btn-outline-primary update" style="margin: 0px 2px;font-size: inherit;" id="update" data-id="' + row.id+ '" >修改</button>' +
                                    '<button type="button" class="btn btn-success forbid" style="margin: 0px 2px;font-size: inherit;" id="forbidById" data-id="' + row.id+ '" >已启用</button>' +
                                    '<button type="button" class="btn btn-outline-danger delete" style="margin: 0px 2px;font-size: inherit;" id="deleteById" data-id="' + row.id + '">删除</button>';
                            }
                    ...
                    if (res.rows.length > 0) {
                        ...
                        for (var i = 0; i < res.rows.length; i++) {
                            ...
                            var obj = {
                                ...
                                segments: (row.upgAirlineBaseSets && row.upgAirlineBaseSets.map(function(item) {
    ...
    function getAirlineInfos() {
        return $.ajax({
           // url: '/airlineInfoManage/getAirlineInfoList?rand=' + Math.random(),
    ...
    function generateDetailTabContentTemp(arr, tabId, name, active) {
        ...
        return container.replace(/\{\{tabId\}\}/g, tabId).replace('{{content}}', res).replace('{{active}}', active ? 'active' : '');
    }

    function generateFormTabContentTemp(arr, tabId, name, active) {
        var container =
            '<div class="tab-pane J-tab fade show {{active}}" id="{{tabId}}" role="tabpanel"><div class="checkbox-list J-checkbox-list" data-type=""><div class="form-row" style="padding:10px 0"><div class="col-6"><input type="text" class="form-control" style="margin-right:8px;font-size: 12px" name="search-'+name+'"></div><div class="col"><button class="btn btn-primary J-search" style="font-size: 12px">搜索</button><div class="btn btn-primary J-reset" style="font-size: 12px">重置</div></div></div>{{content}}<p class="no-result J-no-result">抱歉，未查询到相关航线</p></div></div>';
        var checkTemp =
        ...
        var res = '';
        for (var i = 0; i < arr.length; i++) {
            var temp = checkTemp.replace(/\{\{name\}\}/g, name).replace(/\{\{label\}\}/g, arr[i]);
            res += temp;
        }
        res =
            ...
            segments.push({
                ...
                countryName:
                    data.upgAirlineBaseSets[i].intFlag === 'I'
                        ? data.upgAirlineBaseSets[i].countryName || ''
                        : '',
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter: function(value, row) {
                            var temp;
                            if(null == value){
                                temp = '<input type="text" class="form-control" name="{{name}}" id="businessClassSeatNum" value="" oninput = "value=value.replace(/[^\\d]/g,\'\')">';
                            }else{
                                temp ='<input type="text" class="form-control" name="{{name}}" id="businessClassSeatNum" value="{{value}}" oninput = "value=value.replace(/[^\\d]/g,\'\')">';
                            }
                            return temp.replace(/\{\{name\}\}/, row.planeTypeName || '').replace(/\{\{value\}\}/, value);
                        }
                    },
                    {
            ...
            .bootstrapTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        formatter: function(value) {
                            return (
                                '<input type="text" class="form-control" name="CNY" id="basePrice" value="' + (value?value:'') + '" oninput = "value=value.replace(/[^\\d]/g,\'\')">'
                            );
                        }
                    }
                ],
    ...
    function toggleInlandAndNation() {
        //国际 国内航线切换
        $('.J-air-table-inland').on('click', function(e) {
        ...
        $('.J-air-table-nation').on('click', function(e) {
            var $parent = $(this).closest('.air-table');
    ...
    var formModal = {
        ...
        renderByData: function(data) {
            ...
            if (data) {
                //id显示
                $('.J-id').show().find('input').val(data.id);
                //名称显示
            ...
            $priceTable.bootstrapTable('load',
                data.currency.filter(function(cur) {
                    return cur.code === 'CNY';
                })
            );
            $('#modalAirlinePriceBaseSetName').val(data.paramId);
        }
    };
    ...
    var detailModal = {
        ...
        renderByData: function(data) {
            ...
            $detailPriceTable.bootstrapTable('load', data.currency);
        }
    };

    function modalWriteBack(id,target) {
        //根据id查询数据
        axios.post("/upGradeBaseSet/query",{
            id:id
        }).then(function (resp) {
